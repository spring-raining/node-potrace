...
function Potrace (options) {
  ...
  this._pathlist = [];
  this._fillingBitmapList = [];
  this._nestFlagData = null;
...
Potrace.prototype = {
  ...
  _bmToPathlist: function() {
    ...
    function xorPath(path){
      var y1 = path.pt[0].y,
        ...
        x, y, maxX, minY, i, j,
        indx;
      var filling = new Bitmap(blackMap.width, blackMap.height);
      ...
      for (i = 1; i < len; i++) {
        ...
        if (y !== y1) {
          ...
          for (j = x; j < maxX; j++) {
            ...
            if (blackMap.data[indx]) {
              blackMap.data[indx] = 0;
              filling.data[indx] += 1;
              self._nestFlagData.data[indx] += 1;
            } else {
              blackMap.data[indx] = 1;
              filling.data[indx] -= 1;
              self._nestFlagData.data[indx] -= 1;
            }
          }
    ...
    // it reverses directions of inner path to deal with non-zero winding rule.
    function reverseInnerPath(path) {
      var i;
      var pt = path.pt[0];
      if (self._nestFlagData.getValueAt(pt.x, pt.y) % 2 !== 1) {
        return;
      }
      const newPt = [pt];
      for (i = path.len - 1; i > 0; i--) {
        newPt.push(path.pt[i]);
      }
      path.pt = newPt;
    }

    function linkInnerPath() {
      var i, j;
      var innerIndices = [];
      var areaIndices = self._pathlist.map(function (path, i) {
        return [i, path.area];
      });
      areaIndices.sort(function (a, b) {
        return b[1] - a[1];
      });
      for (i = 1; i < areaIndices.length; i++) {
        var path = self._pathlist[areaIndices[i][0]];
        var pt = path.pt[0];
        for (j = i - 1; j >= 0; j--) {
          var outer = self._pathlist[areaIndices[j][0]];
          var outerFilling = self._fillingBitmapList[areaIndices[j][0]];
          if (outerFilling.getValueAt(pt.x, pt.y) > 0) {
            if (innerIndices.indexOf(j) < 0) {
              outer.hole.push(path);
              path.outerPath = outer;
              innerIndices.push(i);
            }
            break;
          }
  ...
  _processLoadedImage: function(image) {
    ...
    // already initialized in 0
    this._nestFlagData = new Bitmap(image.bitmap.width, image.bitmap.height);
